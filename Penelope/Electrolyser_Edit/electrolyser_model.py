class ElectrolyserPython:
    def __init__(self, eff, resolution, term_eff, rated_power, ramp_up_time=30):
        self.p_in = None
        self.h_out = None
        self.eff = eff
        ...
        self.ramp_up_time = ramp_up_time  # Ramp up/down time in minutes
        self.ramp_rate = rated_power / ramp_up_time  # Maximum power change per minute
        self.p_in_last = 0  # Power input during the last step

    def ramp_rate_limit(self, desired_power):
        # Limit the change in power based on the ramp rate, considering the ramp up/down time
        power_change = desired_power - self.p_in_last
        max_power_change = self.ramp_rate * self.resolution  # Adjusted for the simulation resolution

        if abs(power_change) > max_power_change:
            if power_change > 0:
                desired_power = self.p_in_last + max_power_change
            else:
                desired_power = self.p_in_last - max_power_change
    ...
    def electrolyser(self, flow2e, temperature=15, pressure=100):
        ...
        if e_consume > 0:
            ...
            h_mass = out / 1000  # kg of hydrogen produced
        ...
        self.p_in_last = e_consume  # Update the last power input
        return re_params
